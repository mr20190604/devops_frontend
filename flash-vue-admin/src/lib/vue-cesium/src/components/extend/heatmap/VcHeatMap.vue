<template>
  <i :class="$options.name" style="display: none !important">
    <vc-entity v-if="type === 1" ref="childRef" :show="show">
      <vc-graphics-rectangle :coordinates="coordinates" :material="material" />
    </vc-entity>
    <vc-primitive-ground v-else-if="type === 0" ref="childRef" :appearance="appearance" :release-geometry-instances="false" :show="show">
      <vc-instance-geometry>
        <vc-geometry-rectangle :rectangle="coordinates" />
      </vc-instance-geometry>
    </vc-primitive-ground>
    <vc-layer-imagery v-else-if="type === 2" ref="childRef" :show="show">
      <vc-provider-imagery-tile-single :rectangle="coordinates" :url="layerUrl" />
    </vc-layer-imagery>
  </i>
</template>
<script>
  ...
  watch: {
    changeProps: {
      async handler(val, oldValue) {
        ...
        // JSON.stringify(val.data) !== JSON.stringify(oldValue.data) &&
        this.setWGS84Data(val.min, val.max, val.data)
