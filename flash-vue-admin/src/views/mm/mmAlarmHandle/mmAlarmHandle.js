  ...
  methods: {
    ...
    reset() {
      for (const key in this.listQuery) {
        if (key != 'limit' && key != 'page' && key != 'isAudit' && key != 'auditStatus' && key != 'personId') {
          this.listQuery[key] = ''
        }
      }
      this.fetchData()
    ...
    handleSelectionChange(selection) {
      ...
      this.clearEchart()
      const time = new Date(record.alarmTime)
      ...
      if (this.modelTime.length > 0) {
        ...
        const endMinitus = endDate.getMinutes()
        while (true) {
          ...
          if (startMinitus >= 60) {
            ...
            if (startHour >= 24) {
              ...
              if (startMonth < endMonth) {
                const startMonthDate = new Date(year, startMonth, 0).getDate()
