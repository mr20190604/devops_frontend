<template>
  <div class="app-container">
    ...
    <div class="table-list paddingTop12">
      ...
      >
        ...
        <el-table-column label="操作" align="center" width="268">
          <template slot-scope="scope">
            <el-button v-permission="['/mmBasEquipment/curve']" type="text" size="mini" icon="el-icon-s-marketing" @click.native="viewCurve(scope.row)">监测曲线</el-button>
            <el-button v-permission="['/mmBasEquipment/thresholdSearch']" type="text" size="mini" icon="el-icon-search" @click.native="thresholdView(scope.row)">阈值查询</el-button>
            <el-button v-permission="['/mmBasEquipment/location']" type="text" size="mini" icon="el-icon-location-outline">定位</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <threshold :threshold-form="thresholdForm" :edit-flag="true" :equipment-code="equipCode" />
    </el-dialog>
    ...
    >
      <div align="center">
        <v-chart ref="myEchart" :options="lineData" />
      </div>
    </el-dialog>
  </div>
</template>
